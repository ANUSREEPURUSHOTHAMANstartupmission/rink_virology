<script>
  ...
  <svelte:window bind:scrollY={y}/>
   ...
   flex sm:items-center
    ...
    z-[100] container-full bg-transparent
           {scrolled ? ' top-0 shadow-md' : ' text-black  md:mt-10'}


   ">
    <div class="w-full  mx-auto  flex justify-between px-4 bg-white rounded-md auto-container">
     <div class=" md:px-2 m-0 font-medium gap-2 w-fit p-2 rounded-md flex items-center justify-center ">
     
        <a href="/" >
          <img
          src="{scrolled ? '/img/logo1.svg' : '/img/logo1.svg'}"
          alt="logo"
          class="items-center justify-center h-14"
        />
     ...
     <main class=" flex  items-start px-7 my-auto">
        <div style="font-family: Museo-Sans, sans-serif;" class="flex my-auto  {scrolled?'':''}">
            ...
            <a href="/#Highlights"  class="md:flex page-scroll cursor-pointer hidden px-4 py-3  md:py-1 uppercase   text-sm hover:text-green-900">Highlights</a>

            
        </div>
      
        <div class="flex  items-center justify-center  bg-white my-auto spl_cursor md:hidden flex ">   
             <div on:click="{()=> menu_open = !menu_open }" class="z-[100] items-center justify-center my-auto spl_cursor md:pt-4 pt-1   absolute cursor-pointer top-3 md:right-8 right-4 w-10 h-10 md:text-xl text-xs ml-auto transition duration-150  py-1  -solid -transparent rounded bg-transparent block outline-none focus:outline-none" type="button">
   ...
   flex items-center
    ease-in-out transition-all transform duration-300
   -translate-x-full  fixed z-[100]
    {menu_open?'translate-x-0':''}
   ">
     <div class="flex bg-white top-4 absolute spl_cursor">   
        <button on:click="{()=> menu_open = !menu_open }" class="z-[100] spl_cursor bg-white cursor-pointer fixed right-5 w-10 h-10 text-xl ml-auto transition duration-150 leading-none px-2 py-1  -solid -transparent rounded bg-transparent block outline-none focus:outline-none" type="button">
    ...
    <div class="w-fit container-full mx-auto sm:items-center sm:justify-between">
        <div class="mb-8 gap-2  font-medium sm:bg-transparent p-5 sm:p-0 rounded-md flex items-center justify-center">
         ...
         <nav>
            <ul class=" md:text-sm gap-2 font-data">
                    ...
                    <li on:click="{()=> menu_open = !menu_open }" class="px-5 py-2 transform hover:bg-gray-400 transition duration-500 hover:scale-100 md:hidden flex">
                        <a href="/#Highlights"  class="page-scroll cursor-pointer  font-bold uppercase ">Highlights</a>
                    </li>
